...
import { h as ProtocolError, T as TimeoutWaitingForResponseErrorCode, i as utf8ToBytes, E as ExternalError, M as MissingRootKeyErrorCode, k as Certificate, l as lookupResultToBuffer, m as RequestStatusResponseStatus, U as UnknownError, n as RequestStatusDoneNoReplyErrorCode, p as RejectError, q as CertifiedRejectErrorCode, s as UNREACHABLE_ERROR, I as InputError, t as InvalidReadStateRequestErrorCode, w as ReadRequestType, x as Principal, y as IDL, z as MissingCanisterIdErrorCode, A as HttpAgent, D as encode, Q as QueryResponseStatus, F as UncertifiedRejectErrorCode, G as isV3ResponseBody, J as isV2ResponseBody, K as UncertifiedRejectUpdateErrorCode, L as UnexpectedErrorCode, N as decode, O as Subscribable, V as pendingThenable, W as resolveEnabled, X as shallowEqualObjects, Y as resolveStaleTime, Z as noop$1, _ as environmentManager, $ as isValidTimeout, a0 as timeUntilStale, a1 as timeoutManager, a2 as focusManager, a3 as fetchState, a4 as replaceData, a5 as notifyManager, a6 as hashKey, a7 as getDefaultState, r as reactExports, a8 as shouldThrowError, a9 as useQueryClient, aa as useInternetIdentity, ab as createActorWithConfig, ac as Service, j as jsxRuntimeExports } from "./index-DSY_0Yw_.js";
...
function mergeWithSample(backendData, sample) {
...
function useChannels() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const raw = await actor.getChannels();
...
function useTracks(channelId) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const raw = await actor.getTracks(BigInt(channelId));
...
function useFavorites() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!actor) return [];
      try {
        const raw = await actor.getFavorites();
        return raw.map((t) => ({
          id: bigintToNumber(t.id),
          title: t.title,
          artist: t.artist,
...
function useAddFavorite() {
  ...
  return useMutation({
    mutationFn: async (trackId) => {
      if (!actor) return;
      await actor.addFavorite(BigInt(trackId));
    },
...
function useRemoveFavorite() {
  ...
  return useMutation({
    mutationFn: async (trackId) => {
      if (!actor) return;
      await actor.removeFavorite(BigInt(trackId));
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["favorites"] })
  });
}
